...
describe('Prisma questions repository (E2E)', () => {
  ...
  it('should reset qustion details cache when saving the question', async () => {
    const user = await studentFactory.makePrismaStudent({})
    
    const newSlug = Slug.create(randomUUID());
    const question = await questionFactory.makePrismaQuestion({
      authorId: user.id,
      slug: newSlug
    })
    ...
    const cached = await cacheRepository.get(`question:${newSlug}:details`)
